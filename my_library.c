...
void kaleidoscope(uint8_t *yuvbuf_in, int width, int height,
  ...
  for (int h = height - 1; h >= 0; h -= 2) {
    ...
    for (int w = width_mid - width_length; w <= width_mid + width_length; w += 2) {
      ...
      // Assign pixel values to the 6 o'clock triangle
...
void clone_pixel(uint8_t *yuvbuf, int width, int height, int w, int h,
  ...
  optimising color smoothness and in conjunction with with the considerations
  item as described in item (6) below */
  for (int s = 1; s < n_sectors * 2; ++s) {

    // 3. Add the rotation factor to the angle and covering all quadrants
    theta = (s % 2 != 0) ? theta + fi * s : theta - fi * s;

    // 4. Convert from polar back to cartesian and print pixel at new position
    h_dst = r * sin(theta) + height / 2;
    w_dst = r * cos(theta) + width / 2;

    /* 6. Write each pixel in other sectors at the respective place and in
    close proximity to avoid trigonometric function pitholes occuring at π/2 */
    for (int i = 0; i < 3; ++i) {
      uv_idx = (h_dst / 2) * (width / 2) + (w_dst / 2);
